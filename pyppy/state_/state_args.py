...
on the attributes of the global stateuration we just registered. For
...
def fill_state_args(*args_to_be_filled: str) -> Callable:
    ...
    args_to_be_filled : str
        ...
        are filled from the global states attributes. The Decorator returned
            ...
            if len(args) > 0:
                raise OnlyKeywordArgumentsAllowedException(
                    (
                        ...
                        f"function defined with the {fill_state_args.__name__} "
                        f"decorator."
                    )
                )
